...
- Learn how to recognise and compile the basic components of a Quarto file
...
Creating a report as a web page (which is an html file) using Quarto makes things easier.
...
but we recommend that you use Quarto, which mixes Markdown with a programming language (in this case, R).
[Markdown](https://www.markdownguide.org/) is a light-weight mark-up language for creating web pages.

You will use [`quarto`](https://quarto.org/docs/computations/r.html) to create your documents.
`quarto` will call `knitr` to generate the output format.
...
It is possible to use R Markdown directly and not use the newer Quarto
interface.  This lesson will focus on using Quarto, but if you would like
to try creating an R Markdown document, within RStudio,
click File → New File → R Markdown and
you'll get a dialog box like this:
...
You *compile* the Quarto document to an html webpage by clicking
the "Render" button in the upper-left.

:::::::::::::::::::::::::::::::::::::::  challenge

## Challenge 1

Create a Quarto document. Delete all of the R code chunks
...
and <code>\`\`\`</code>. You should give each chunk
a unique label by adding the line `#| label: chunk\_label` after the start of each chunk, as they will help you to fix errors and, if any graphs are
produced, the file names are based on the name of the code chunk that
produced them. You can create code chunks quickly in RStudio using the shortcuts
...
the R code is executed and replaced by both the input and the output;
...
<img src="fig/14-reproducible-documents-rendered-qmd_to_html_fig-1.png" alt="" style="display: block; margin: auto auto auto 0;" />
...
repeatedly; for this, you can set *global* chunk options using the `knitr` tag in the header, like so:
...
Replace `format: html` by `format: pdf` or `format: docx`,
...
software (you'll only need to do this once) and then when you generate a pdf `tinytex`
...
You can switch between source and visual modes by pressing the buttons
in the top left of the RStudio window
![](fig/source_visual_buttons.png){alt='Buttons for changing between source and visual editing modes in RStudio'}
...
- Convert these documents into web pages and other formats.
