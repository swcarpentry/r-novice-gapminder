...
> > **Step 1**: We first obtain a logical vector describing which element of `gapminder$year` is equal to `2002`:
> > ```{r ch10pt1-sol, eval=FALSE}
> > gapminder[(gapminder$year == 2002),]
> > ```
> > **Step 2**: Then, we count the number of rows of the data.frame `gapminder` that correspond to the 2002:
...
> > rows2002_number >= 1
> > ```
> > **Step 3**: Finally, putting all together, we obtain:
> > ```{r ch10pt4-sol, eval=FALSE}
> > if(nrow(gapminder[(gapminder$year == 2002),]) >= 1){
> >    print("Record(s) for the year 2002 found.")
> > }
> > ```
> > **Alternative Solutions**: All this can be done more quickly with either the `%in%` operator or the `any()` function.
> > Using the `%in%` operator, the logical condition can be expressed as:
> > ```{r for (i in 1:10) {
