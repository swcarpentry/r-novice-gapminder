...
> >
> >lifeExp_bycountry$country[which.max(lifeExp_bycountry$mean_lifeExp)]
> >lifeExp_bycountry$country[which.min(lifeExp_bycountry$mean_lifeExp)]
> >```
> Another way to do this is to use the `dplyr` function `arrange()`, which has 
> similar syntax to other functions from the `dplyr` package.
> >```{r}
> >lifeExp_bycountry %>%
