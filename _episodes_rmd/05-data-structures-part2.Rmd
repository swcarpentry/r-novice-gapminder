...
data are consistent in type throughout each of the columns. 
As such, if we want to add a new column, we can start by making a new vector:
...
Why didn't this work? To keep the rectangular format, 
R wants to see one element in our new column
...
We see that we have successfully added a row.

```{r}
cats
```

Let us change the data frame slightly, and again try to add a new row. 
The change we will make is to turn the `coat` column into a _factor_ 
...
Next, we try to add another row to the new data frame.

```{r}
newRow <- list("white", 4.3, 1, 7)
cats <- rbind(cats, newRow)
```

Now, our attempt to use the `rbind()` function returns a warning.  Recall that, unlike errors, warnings do not necessarily stop a function from performing its intended action.  You can confirm this by taking a look at the `cats` data frame.

```{r}
cats
```

Notice that not only did we successfully add a new row, but there is `NA` in the column *coats* where we expected "white" to be.  Why did this happen?

## Factors

For an object containing the data type `factor`, each different value represents what is called a `level`. In our case, the `factor` "coat" has 4 levels: "black", "calico", "tabby", and "tortoiseshell". R will only accept values that match one of the levels. If you add a new value, it will become `NA`.

The warning is telling us that we unsuccessfully added "white" to our
*coat* factor, but 4.3 (a numeric), 1 (an integer), and 7 (a numeric) were
...
"white" *coat*, add "white" as a possible *level* of the factor:

```{r}
levels(cats$coat)
levels(cats$coat) <- c(levels(cats$coat), "white")
cats <- rbind(cats, list("white", 4.3, 1, 7))
```

Alternatively, we can keep the column as a character vector instead of 
turning it into a factor; we lose the
...
> > 3. `as.numeric(human_age)` yields `1 2 3 5 4 4` because factors are stored as integers (here, 1:5), each of which is associated with a label (here, 14, 21, 35, 49, and 63). Converting the factor to a numeric vector gives us the underlying integers, not the labels. If we want the original numbers, we need to convert `human_age` to a character vector (using `as.character(human_age)`) and then to a numeric vector (why does this work?).
> {: .solution}
{: .challenge}
...
first attempt to add a "white" cat to the data frame we have accidentally
added a garbage row:
...
Notice the comma with nothing after it to indicate that we want to drop the entire fifth row.

Note: we could also remove all new rows at once by putting the row numbers
inside of a vector: `cats[c(-4,-5,-6), ]`
...
Note that columns containing character values are read as characters. 
This is the default behaviour in R version 4.0.0 and newer. In earlier versions, 
reading text files would automatically convert character columns to 
factors. This illustrates that it is important to verify that all data 
is indeed read as we intend it to! If we would like character columns 
to be converted to factors with R version 4.0.0 or newer, we need to 
specify that explicitly when reading the csv file:
...
Now, the two character columns are indeed represented as factors. 
