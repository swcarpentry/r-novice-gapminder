...
to ensure that functions only work in their intended use-cases. This  is a part
of the concept of _defensive programming_. Defensive programming encourages us
to frequently check conditions and throw an error if something is
wrong. These checks are referred to as assertion statements because we want to
assert some condition is `TRUE` before proceeding. This makes it easier to
debug because we have a better idea of where the error originated.
...
`stop`. For example, since the arugment `temp` must be a `numeric` vector, we
...
> ## Challenge 3
>
> Use defensive programming to ensure that our `fahr_to_celsius` function kicks
> in to full effect only when the `temp` argument is specified appropriately.
>
>
...
Now, we're going to define a function that calculates the Gross Domestic
Product of a nation from the data available in our dataset:
...
Then it calculates the GDP for whatever subset emerges from the previous two
steps. The function then adds the GDP as a new column to the subsetted data and
returns this as the final result.
