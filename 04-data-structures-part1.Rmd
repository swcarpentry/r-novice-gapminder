...
```{r}
cats <- read.table(file="data/feline-data.csv", header=TRUE, sep=",")
cats
```

The extra arguments after the filename help R get its hands around our data; `header=TRUE` lets it know there's a title row, and `sep=","` lets it know our columns are separated by `,` characters. Note that if you like to maintain the English idiom and put spaces after your commas, you'll need to use `sep=", "`; either is fine, as long as you're consistent.
...
As you can see, some surprising things can happen when R forces one basic data type into another! Nitty-gritty of type coercion aside, the point is: if you're data doesn't look like what you thought it was going to look like, type coercion may well be to blame; make sure everything is the same type in your vectors and your columns of data.frames, or you will get nasty surprises!
...
> try using the help for `read.table` to figure out how to keep text columns as character vectors instead of factors; 
> then write a command or two to show that coats actually is a character vector when loaded in this way.
>
...
We see that data.frames look like lists 'under the hood' - this is because a data.frame is really a list of vectors, as they have to be - in order to hold those columns that are a mix of vectors and factors, the data.frame needs something a bit more flexible than a vector to put all the columns together into a familiar table.
...
> x <- [11:20]
> firstFive <- head(x, 5)
> thirdToFifth <- tail(firstFive, 3)
> names(thirdToFifth) <- c('S', 'W', 'C')
> ```
>

> ## Challenge 2 {.challenge}
> ```{r}
> cats <- read.table(file="data/feline-data.csv", header=TRUE, sep=",", stringsAsFactors=FALSE)
> str(cats$coat)
> ``` 
