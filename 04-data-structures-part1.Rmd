...
>```
> Try to get character vector of sex values from `sex` factor
> and numeric vector from `age`.
> Does using `as.character()` and `as.numeric()` return results you expect?
...
> extracting character vector from factor works as we would expected:
> ```{r, eval=FALSE}
> as.character(sex)
> [1] "M" "F" "F" "M" "M" "F"
> ```
> but not for factor with numerical values
> ```{r, eval=FALSE}
