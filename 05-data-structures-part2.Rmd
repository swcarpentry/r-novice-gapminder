...
At this ponit, you've see it all - those are all the basic data types and data structures in R; everything you do will be working with those things. But a whole lot of the time, the star of the show is going to be the `data.frame` - that table that we started with that information from a CSV gets dumped into when we load it. In this lesson, we'll learn a few more things about working with `data.frame`.

We learned last time that the columns in a data.frame were vectors, so that our data is reliable and consistent throughout the column. As such, if we want to add a new column, we need to start by making a new vector:
...
Another thing to look out for has emerged - when R creates a factor, it only allows whatever is originally there when our data was first loaded, which was 'black', 'calico' and 'tabby' in our case. Anything new that doesn't fit into one of its little bins is rejected as nonsense, until we explicitly add that as a *level* in the factor:

```{r}
levels(cats$coat)
levels(cats$coat) <- c(levels(cats$coat), 'tortoise')
cats <- rbind(cats, list("tortoise", 3.3, TRUE, 9))
```
...
The key to remember when adding data to a data.frame is that *columns are vectors, and rows are lists.*
...
```{r}
gapminder <- read.table("data/gapminder-FiveYearData.csv", header=TRUE, sep=',')
```
...
```{r}
typeof(gappy$year)
typeof(gappy$lifeExp)
typeof(gappy$country)
str(gappy$country)
```
...
When `length` gave us 6, it's because gapminder is a list of 6 columns. To get the number of rows and columns in our dataset, try:
